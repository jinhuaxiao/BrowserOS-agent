...
namespace {
Vector<String> ParseSuffixes(const std::string& suffixes) {
  Vector<String> extensions;
  std::string current;
  for (size_t i = 0; i <= suffixes.size(); ++i) {
    if (i == suffixes.size() || suffixes[i] == ',') {
      // Trim whitespace
      size_t start = current.find_first_not_of(" \t");
      size_t end = current.find_last_not_of(" \t");
      if (start != std::string::npos && end != std::string::npos) {
        extensions.push_back(
            String::FromUTF8(current.substr(start, end - start + 1)));
      }
      current.clear();
    } else {
      current.push_back(suffixes[i]);
    }
  }
  return extensions;
}
...
DOMPluginArray::DOMPluginArray(LocalDOMWindow* window) : window_(window) {
  ...
  if (config.IsEnabled() && config.HasPluginOverride()) {
    const auto& plugins = config.GetPlugins();
    for (const auto& plugin : plugins) {
      auto* plugin_info = MakeGarbageCollected<PluginInfo>(
          String::FromUTF8(plugin.name), String::FromUTF8(plugin.filename),
          String::FromUTF8(plugin.description),
          /*background_color=*/Color::kTransparent,
          /*may_use_external_handler=*/false);
      for (const auto& mime : plugin.mime_types) {
        Vector<String> extensions = ParseSuffixes(mime.suffixes);
        auto* mime_info = MakeGarbageCollected<MimeClassInfo>(
            String::FromUTF8(mime.type), String::FromUTF8(mime.description),
            *plugin_info, extensions);
        plugin_info->AddMimeType(mime_info);
      }
      dom_plugins_.push_back(MakeGarbageCollected<DOMPlugin>(window, *plugin_info));
    }
    return;
  }
  ...
  if (config.IsEnabled() && config.HasPluginOverride()) {
    for (const auto& plugin : dom_plugins_) {
      for (unsigned j = 0; j < plugin->length(); j++) {
        mimetypes.push_back(plugin->item(j));
      }
    }
    return mimetypes;
  }
...
bool DOMPluginArray::IsPdfViewerAvailable() {
  ...
  if (config.IsEnabled() && config.HasPluginOverride()) {
    for (const auto& plugin : dom_plugins_) {
      for (unsigned j = 0; j < plugin->length(); j++) {
        DOMMimeType* mime_type = plugin->item(j);
        if (mime_type && mime_type->type() == "application/pdf")
          return true;
      }
    }
    return false;
