...
function setupSearch() {
  ...
  input.addEventListener('input', () => {
    ...
    const matches = companiesData.filter(
      (c) =>
    ...
    results.querySelectorAll('.search-result-item').forEach((item) => {
      item.addEventListener('click', () => {
        ...
        var company = companiesData.find((c) => c.company === name)
        if (company) {
...
function renderOverview() {
  ...
  if (activeSector !== 'all') {
    if (activeSector === 'other') {
      filtered = companiesData.filter((c) => !mainSectors.includes(c.sector))
    } else {
      filtered = companiesData.filter((c) => c.sector === activeSector)
    }
  }
...
function renderDetailView(company) {
  ...
  company.metrics.forEach((m, i) => {
    ...
    var labels = m.data.map((d) => d.year)
    var values = m.data.map((d) => d.value)
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (ctx) => formatValue(ctx.parsed.y, m.unit),
            },
          },
        },
        scales: {
  ...
  // Compare buttons
  grid.querySelectorAll('.chart-compare-btn').forEach((btn) => {
...
function renderCompareView() {
  ...
  sidebar.querySelectorAll('.compare-category').forEach((btn) => {
    btn.addEventListener('click', () => {
      sidebar.querySelectorAll('.compare-category').forEach((b) => {
        b.classList.remove('active')
      })
      btn.classList.add('active')
      var groupId = btn.getAttribute('data-group')
      var group = compareGroups.find((g) => g.id === groupId)
      if (group) {
...
function buildCompareGroups(metricMap) {
  ...
  var categories = [
    ...
    },
    {
      ...
      keywords: ['keytruda', 'humira', 'skyrizi', 'oncology', 'pharmaceutical'],
    },
    {
      id: 'transactions',
    ...
    },
    {
      ...
      keywords: ['store count', 'warehouse', 'branch', 'fulfillment center'],
    },
    {
      id: 'backlog',
      label: 'Order Backlog',
  ...
  categories.forEach((cat) => {
    ...
    companiesData.forEach((c) => {
      c.metrics.forEach((m) => {
        ...
        var isMatch = cat.keywords.some((kw) => mLower.includes(kw))
...
function renderComparisonChart(group) {
  ...
  var datasets = group.companies.map((c, i) => {
    ...
    return {
      ...
      data: years.map((y) => (dataMap[y] !== undefined ? dataMap[y] : null)),
      borderColor: color,
