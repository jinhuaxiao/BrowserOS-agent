...
int SSLClientSocketImpl::Init() {
  ...
  // BrowserOS: Read TLS profile from command line for JA3 fingerprint control
  std::string tls_profile = "chrome";
  {
    const auto* cmd_line = base::CommandLine::ForCurrentProcess();
    if (cmd_line && cmd_line->HasSwitch("tls-profile")) {
      tls_profile = cmd_line->GetSwitchValueASCII("tls-profile");
    }
  }
  ...
  if (tls_profile == "firefox") {
    ...
    supported_groups = {SSL_GROUP_SECP256R1, SSL_GROUP_SECP384R1,
                        SSL_GROUP_SECP521R1, SSL_GROUP_X25519};
    key_shares = {SSL_GROUP_SECP256R1};
  } else {
    // Chrome default
    supported_groups = context_->config().GetSupportedGroups();
    key_shares = context_->config().GetSupportedGroups(/*key_shares_only=*/true);
  }
  ...
  // BrowserOS: Firefox allows ECDSA+SHA1 for compatibility
  std::string command(tls_profile == "firefox" ? "ALL:!aPSK:!3DES"
  ...
  // BrowserOS: Only Chrome uses ECH GREASE; Firefox/Safari do not
  if (context_->config().ech_enabled && tls_profile == "chrome") {
  ...
  // BrowserOS: Chrome permutes extensions; Firefox/Safari use fixed order
  SSL_set_permute_extensions(ssl_.get(), tls_profile == "chrome" ? 1 : 0);
