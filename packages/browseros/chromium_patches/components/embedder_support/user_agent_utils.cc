...
// BrowserOS: Helper functions for building UA metadata from fingerprint config
...
std::string NormalizePlatformFromUA(const std::string& platform,
                                    const std::string& ua) {
  if (!platform.empty()) {
...
std::string DetectArchitectureFromUA(const std::string& ua) {
  std::string lower = base::ToLowerASCII(ua);
...
std::string DetectBitnessFromUA(const std::string& ua) {
  std::string lower = base::ToLowerASCII(ua);
  if (lower.find("arm64") != std::string::npos ||
      lower.find("aarch64") != std::string::npos ||
...
bool DetectWow64FromUA(const std::string& ua) {
  std::string lower = base::ToLowerASCII(ua);
...
blink::UserAgentMetadata BuildUserAgentMetadataFromConfig(
    const blink::FingerprintConfig& config,
  ...
  metadata.bitness = DetectBitnessFromUA(ua);
  metadata.wow64 = DetectWow64FromUA(ua);
...
blink::UserAgentMetadata GetUserAgentMetadata(const PrefService* pref_service,
                                              bool only_low_entropy_ch) {
  // BrowserOS: Return metadata derived from fingerprint config if available
  EnsureFingerprintConfigLoaded();
  const auto& fp_config = blink::FingerprintConfig::GetInstance();
  if (fp_config.IsEnabled() && !fp_config.GetUserAgent().empty()) {
    return BuildUserAgentMetadataFromConfig(fp_config, only_low_entropy_ch);
  }
