...
ClashOfGptsCoordinator::~ClashOfGptsCoordinator() {
  // Only save state if browser is still valid (not already cleaned up by
  // OnBrowserRemoved). The ScopedObservation objects auto-unregister.
  if (browser_) {
    SaveState();
    CleanupWebContents();
    browser_ = nullptr;
...
void ClashOfGptsCoordinator::CleanupWebContents() {
  ...
  // Clear browser pointer in window before destroying it to prevent
  // dangling raw_ptr detection during shutdown.
  if (window_) {
    window_->ClearBrowser();
  }
...
void ClashOfGptsCoordinator::OnBrowserRemoved(Browser* browser) {
  if (browser == browser_) {
    SaveState();
    CleanupWebContents();
    // Null out browser_ to prevent dangling raw_ptr. The destructor
    // will skip SaveState() since browser_ is already null.
    browser_ = nullptr;
  }
}

void ClashOfGptsCoordinator::OnProfileWillBeDestroyed(Profile* profile) {
  if (browser_ && profile == browser_->profile()) {
    SaveState();
    CleanupWebContents();
    browser_ = nullptr;
  }
