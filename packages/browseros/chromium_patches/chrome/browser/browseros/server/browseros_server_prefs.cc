...
+
+// Proxy server port (alias for MCP port, used by extension)
+const char kProxyServerPort[] = "browseros.server.proxy_port";
+
...
+
+  // Proxy port (alias for MCP port, used by extension)
+  registry->RegisterIntegerPref(kProxyServerPort, kDefaultMCPPort);
+
+  // Extension port
