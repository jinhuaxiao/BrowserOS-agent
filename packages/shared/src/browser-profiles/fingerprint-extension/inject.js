...
(function() {
  ...
  const intlConstructors = ['NumberFormat', 'Collator', 'PluralRules', 'RelativeTimeFormat', 'ListFormat', 'DisplayNames', 'Segmenter'];

  intlConstructors.forEach(name => {
    if (typeof Intl[name] !== 'undefined') {
      const Original = Intl[name];
      const Spoofed = function(locales, options) {
        return new Original(locales || configuredLanguage, options);
      };
      ...
      Spoofed.toString = function() { return 'function ' + name + '() { [native code] }'; };
      Intl[name] = Spoofed;
    }
  });
  ...
  if (config.fonts) {
    ...
    if (document.fonts && document.fonts.check) {
      ...
      document.fonts.check = function(font, text) {
        ...
        const fontFamily = font.replace(/^[\d.]+(?:px|pt|em|rem|%)\s+/, '').replace(/["']/g, '').trim();

        // Check if the font family is in our allowed list
        const isAllowed = allowedFonts.has(fontFamily) ||
                         allowedFonts.has(fontFamily.toLowerCase()) ||
                         // Check for generic font families
                         ['serif', 'sans-serif', 'monospace', 'cursive', 'fantasy', 'system-ui'].includes(fontFamily.toLowerCase());
    ...
    if (document.fonts && typeof document.fonts[Symbol.iterator] === 'function') {
      const originalIterator = document.fonts[Symbol.iterator].bind(document.fonts);
      document.fonts[Symbol.iterator] = function* () {
        for (const fontFace of originalIterator()) {
          if (allowedFonts.has(fontFace.family) || allowedFonts.has(fontFace.family.replace(/["']/g, ''))) {
            yield fontFace;
          }
        }
      };
    ...
    if (document.fonts && document.fonts.forEach) {
      ...
      document.fonts.forEach = function(callback, thisArg) {
        originalForEach(function(fontFace, index, fonts) {
  ...
  if (config.plugins && Array.isArray(config.plugins.items)) {
    function createMimeType(mime, plugin) {
      const mimeType = {
        ...
        toString: function() { return '[object MimeType]'; }
      };
    ...
    function createPlugin(pluginData) {
      ...
      const mimeTypes = Array.isArray(pluginData.mimeTypes) ? pluginData.mimeTypes : [];
    ...
    function createMimeTypeArray(pluginsData, pluginArray) {
      ...
      pluginsData.forEach((pluginData, pluginIndex) => {
        ...
        const mimeList = Array.isArray(pluginData.mimeTypes) ? pluginData.mimeTypes : [];
  ...
  if (config.mediaDevices && navigator.mediaDevices) {
    ...
    if (navigator.mediaDevices.getUserMedia) {
      ...
      navigator.mediaDevices.getUserMedia = function(constraints) {
        if (config.webrtc && config.webrtc.disableWebRTC) {
