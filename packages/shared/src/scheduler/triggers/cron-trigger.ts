...
function parseCronExpression(expression: string): ParsedCron {
  ...
  return {
    ...
    month: parseField(fields[3], 'month', MONTH_NAMES),
    dayOfWeek: parseField(fields[4], 'dayOfWeek', DAY_NAMES),
  };
}
 ...
 */
function getNextValue(current: number, field: CronField, min: number, max: number): number | null {
  if (field.any) {
    return current;
  }
...
export function getNextCronTime(
  ...
  while (iterations < maxIterations) {
    ...
    if (!matchesField(month, parsed.month)) {
      ...
      if (nextMonth === null || nextMonth < month) {
        ...
        next.setMonth(parsed.month.any ? 0 : parsed.month.values[0] - 1);
      } else {
    ...
    const dayMatches =
      ...
      (!parsed.dayOfMonth.any && !parsed.dayOfWeek.any && (dayOfMonthMatches || dayOfWeekMatches));
    ...
    if (!matchesField(hour, parsed.hour)) {
      ...
      if (nextHour === null || nextHour < hour) {
        // Wrap to next day
        next.setDate(next.getDate() + 1);
        next.setHours(parsed.hour.any ? 0 : parsed.hour.values[0]);
      } else {
        next.setHours(nextHour);
      }
      next.setMinutes(parsed.minute.any ? 0 : parsed.minute.values[0]);
      continue;
    }
  ...
  throw new Error(`Could not find next execution time for "${expression}" within 1 year`);
}
...
export function describeCronExpression(expression: string): string {
  try {
    ...
    if (!parsed.month.any) {
      ...
      const months = parsed.month.values.map(m => monthNames[m]);
