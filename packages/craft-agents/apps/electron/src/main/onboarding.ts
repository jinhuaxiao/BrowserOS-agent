...
export function registerOnboardingHandlers(
  ...
  ipcMain.handle(
    ...
    async (_event, mcpUrl: string, accessToken?: string) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, mcpUrl: string) => {
      mainLog.info('[Onboarding:Main] ONBOARDING_START_MCP_OAUTH received', {
        mcpUrl,
      })
      try {
        ...
        const oauth = new CraftOAuth(
          ...
          {
            onStatus: (msg) =>
              mainLog.info('[Onboarding:Main] MCP OAuth status:', msg),
            onError: (err) =>
              mainLog.error('[Onboarding:Main] MCP OAuth error:', err),
          },
        )

        mainLog.info(
          '[Onboarding:Main] Calling oauth.authenticate() - this may open browser and wait...',
        )
    ...
    async (
      ...
      try {
        ...
        if (config.credential && config.authType) {
          mainLog.info(
            '[Onboarding:Main] Saving credential for authType:',
            config.authType,
          )
          if (config.authType === 'api_key') {
            ...
            mainLog.info(
              '[Onboarding:Main] Credentials should already be saved via native OAuth flow',
            )
          }
        } else {
        ...
        mainLog.info(
          '[Onboarding:Main] Existing config:',
          existingConfig ? 'found' : 'not found',
        )
        ...
        if (config.authType) {
          mainLog.info(
            '[Onboarding:Main] Updating authType from',
            newConfig.authType,
            'to',
            config.authType,
          )
        ...
        if (config.anthropicBaseUrl !== undefined) {
          mainLog.info(
            '[Onboarding:Main] Updating anthropicBaseUrl to',
        ...
        if (config.customModel !== undefined) {
          mainLog.info(
            '[Onboarding:Main] Updating customModel to',
            config.customModel,
          )
        ...
        if (config.workspace) {
          ...
          const existingIndex = newConfig.workspaces.findIndex(
            (w) =>
              w.name.toLowerCase() === config.workspace!.name.toLowerCase(),
          )
          const existingWorkspace =
            existingIndex !== -1 ? newConfig.workspaces[existingIndex] : null
          ...
          mainLog.info(
            '[Onboarding:Main] Workspace config:',
            workspace,
            existingWorkspace ? '(updating existing)' : '(new)',
          )

          // Save MCP credentials if provided
          if (config.mcpCredentials) {
            mainLog.info(
              '[Onboarding:Main] Saving MCP credentials for workspace',
            )
          ...
          mainLog.info(
            '[Onboarding:Main] No workspace to create (billing-only update)',
          )
          ...
          if (newConfig.workspaces.length === 0) {
            ...
            mainLog.info(
              '[Onboarding:Main] Auto-creating default workspace:',
              workspaceId,
            )
        ...
        try {
          mainLog.info(
            '[Onboarding:Main] Reinitializing SessionManager auth...',
          )
          await sessionManager.reinitializeAuth()
          mainLog.info('[Onboarding:Main] Reinitialized auth after config save')
        } catch (authError) {
          mainLog.error(
            '[Onboarding:Main] Failed to reinitialize auth:',
            authError,
          )
  ...
  ipcMain.handle(
    ...
    async (_event, authorizationCode: string) => {
      try {
        ...
        const expiresAtDate = tokens.expiresAt
          ...
          : 'never'
        mainLog.info(
          `[Onboarding] Claude OAuth successful (expires: ${expiresAtDate})`,
        )
  ...
  // Import CLI credentials from macOS Keychain
  ipcMain.handle(IPC_CHANNELS.ONBOARDING_IMPORT_CLI_CREDENTIALS, async () => {
    try {
      const creds = getClaudeCliCredentials()
      if (!creds) {
      ...
      mainLog.info('[Onboarding] Imported CLI credentials from keychain')
      return { success: true, token: creds.accessToken }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error'
      mainLog.error('[Onboarding] CLI credential import failed:', message)
      return { success: false, error: message }
    }
  })
}
