...
import type { BrowserType, AvailableBrowser } from '../shared/types';
...
export function registerBrowserProfileHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.BROWSER_PROFILES_GET, async (_event, profileId: string) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.BROWSER_PROFILES_CREATE, async (_event, input: CreateProfileInput) => {
    try {
  ...
  ipcMain.handle(
    ...
    async (_event, profileId: string, input: UpdateProfileInput) => {
      try {
        ...
        if (profile) {
          ipcLog.info(`Updated browser profile: ${profile.name} (${profile.id})`);
        }
        return profile;
      } catch (error) {
  ...
  ipcMain.handle(IPC_CHANNELS.BROWSER_PROFILES_DELETE, async (_event, profileId: string) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.BROWSER_PROFILES_LAUNCH, async (_event, profileId: string) => {
    try {
      ...
      if (result.success) {
        ipcLog.info(`Launched browser for profile: ${profile.name} (PID: ${result.pid})`);
      } else {
        ipcLog.error(`Failed to launch browser for profile ${profileId}: ${result.error}`);
      }
      return result;
    } catch (error) {
  ...
  ipcMain.handle(IPC_CHANNELS.BROWSER_PROFILES_STOP, async (_event, profileId: string) => {
    try {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        if (profile) {
          ipcLog.info(`Regenerated fingerprint for profile: ${profile.name} (${profile.id})`);
        }
        return profile;
      } catch (error) {
        ipcLog.error(`Failed to regenerate fingerprint for profile ${profileId}:`, error);
        throw error;
      }
    }
  );
  ...
  ipcMain.handle(
    ...
    async (_event, inputs: CreateProfileInput[]) => {
      try {
        const profiles = await Promise.all(inputs.map((input) => createProfile(input)));
  ...
  // Get a single proxy
  ipcMain.handle(IPC_CHANNELS.PROXY_POOL_GET, async (_event, proxyId: string) => {
    try {
  ...
  // Create a new proxy
  ipcMain.handle(IPC_CHANNELS.PROXY_POOL_CREATE, async (_event, input: CreateProxyInput) => {
    try {
  ...
  // Delete a proxy
  ipcMain.handle(IPC_CHANNELS.PROXY_POOL_DELETE, async (_event, proxyId: string) => {
    try {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        ipcLog.info(`Imported ${result.success} proxies (${result.failed} failed)`);
        return result;
      } catch (error) {
  ...
  ipcMain.handle(IPC_CHANNELS.PROXY_POOL_CHECK_HEALTH, async (_event, proxyId: string) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.PROXY_POOL_GET_PROFILES_USING, async (_event, proxyId: string) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.PROXY_POOL_TEST_CONNECTION, async (_event, config: { host: string; port: number }) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.PROXY_POOL_DETECT_GEO, async (_event, proxyId: string) => {
    try {
      const geoLocation = await detectAndUpdateProxyGeoLocation(proxyId);
      if (geoLocation) {
        ipcLog.info(`Detected geolocation for proxy ${proxyId}: ${geoLocation.city}, ${geoLocation.country}`);
      }
      return geoLocation;
    } catch (error) {
      ipcLog.error(`Failed to detect geolocation for proxy ${proxyId}:`, error);
      throw error;
    }
  });
  ...
  ipcMain.handle(IPC_CHANNELS.PROXY_POOL_REFRESH_ALL_GEO, async () => {
    try {
      ...
      ipcLog.info(`Refreshed geolocation: ${result.success}/${result.total} successful`);
      return result;
    } catch (error) {
  ...
  // Get a single group
  ipcMain.handle(IPC_CHANNELS.PROFILE_GROUPS_GET, async (_event, groupId: string) => {
    try {
  ...
  // Create a new group
  ipcMain.handle(IPC_CHANNELS.PROFILE_GROUPS_CREATE, async (_event, input: CreateGroupInput) => {
    try {
  ...
  // Delete a group
  ipcMain.handle(IPC_CHANNELS.PROFILE_GROUPS_DELETE, async (_event, groupId: string) => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.PROFILE_GROUPS_GET_PROFILES, async (_event, groupId: string) => {
    try {
  ...
  ipcMain.handle(
    ...
    async (_event, profileId: string, groupId: string | undefined) => {
      try {
        ...
        if (moved) {
          ipcLog.info(`Moved profile ${profileId} to group ${groupId || 'none'}`);
        }
        return moved;
      } catch (error) {
  ...
  ipcMain.handle(IPC_CHANNELS.PROFILE_TEMPLATES_GET, async (_event, templateId: string) => {
    try {
  ...
  ipcMain.handle(
    ...
    async (_event, input: CreateTemplateInput) => {
      try {
        ...
        ipcLog.info(`Created profile template: ${template.name} (${template.id})`);
        return template;
      } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (_event, templateId: string, input: UpdateTemplateInput) => {
      try {
        ...
        if (template) {
          ipcLog.info(`Updated profile template: ${template.name} (${template.id})`);
        }
        return template;
      } catch (error) {
  ...
  // Delete a template
  ipcMain.handle(IPC_CHANNELS.PROFILE_TEMPLATES_DELETE, async (_event, templateId: string) => {
    try {
  ...
  ipcMain.handle(
    ...
    async (_event, templateId: string, overrides?: Partial<CreateProfileInput>) => {
      try {
        const profile = await createProfileFromTemplate(templateId, overrides);
        ipcLog.info(`Created profile from template: ${profile.name} (${profile.id})`);
        return profile;
      } catch (error) {
        ipcLog.error(`Failed to create profile from template ${templateId}:`, error);
        throw error;
      }
    }
  );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        const profiles = await batchCreateFromTemplate(templateId, count, options);
        ipcLog.info(`Batch created ${profiles.length} profiles from template ${templateId}`);
        return profiles;
      } catch (error) {
        ipcLog.error(`Failed to batch create from template ${templateId}:`, error);
        throw error;
      }
    }
  );
...
function listAvailableBrowsers(): AvailableBrowser[] {
  ...
  const browsers: Array<{ path: string; name: string; type: BrowserType }> = [];

  // Define browser paths by platform
  const browserPaths: Record<string, Array<{ path: string; name: string; type: BrowserType }>> = {
    darwin: [
  ...
  if (currentConfig.customBrowserPath && !result.find(b => b.path === currentConfig.customBrowserPath)) {
    result.push({
      name: 'Custom Browser',
