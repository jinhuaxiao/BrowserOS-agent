...
function SessionItem({
  ...
  const resolvedLabels = useMemo(() => {
    if (!item.labels || item.labels.length === 0 || flatLabels.length === 0)
      return []
    return item.labels
      .map((entry) => {
            ...
            >
              ...
              <div className="flex flex-col gap-1.5 min-w-0 flex-1">
                ...
                <div className="flex items-start gap-2 w-full pr-6 min-w-0">
                  ...
                  >
                    {searchQuery
                      ? highlightMatch(getSessionTitle(item), searchQuery)
                ...
                <div className="flex items-center gap-1.5 text-xs text-foreground/60 w-full -mb-[2px] min-w-0">
                  ...
                  >
                    ...
                    {resolvedLabels.map(
                      ({ config: label, rawValue }, labelIndex) => {
                        const color = label.color
                          ? resolveEntityColor(label.color, isDark)
                          : null
                        const displayValue = rawValue
                          ? formatDisplayValue(rawValue, label.valueType)
                          : undefined
                        return (
                          <LabelValuePopover
                            key={label.id}
                            ...
                            onOpenChange={(open) =>
                              setOpenLabelIndex(open ? labelIndex : null)
                            }
                            onValueChange={(newValue) => {
                              // Rebuild labels array with the updated value for this label
                              const updatedLabels = (item.labels || []).map(
                                (entry) => {
                                  const parsed = parseLabelEntry(entry)
                            ...
                            >
                              ...
                              {displayValue ? (
                                <>
                                  ...
                                  <span
                                    className="font-normal truncate min-w-0"
                                    style={{ opacity: 0.75 }}
                                  >
                                    {displayValue}
                                  </span>
                    ...
                    {item.sharedUrl && (
                      <DropdownMenu modal={true}>
                        ...
                        <StyledDropdownMenuContent align="start">
                          <StyledDropdownMenuItem
                            onClick={() =>
                              window.electronAPI.openUrl(item.sharedUrl!)
                            }
                          >
                            <Globe />
                            Open in Browser
                  ...
                  {item.lastMessageAt && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="shrink-0 text-[11px] text-foreground/50 whitespace-nowrap cursor-default">
                          {formatDistanceToNowStrict(
                  ...
                  <StyledDropdownMenuContent align="end">
                    <DropdownMenuProvider>
                      <SessionMenu
                        ...
                        onLabelsChange={
                          onLabelsChange
                            ? (newLabels) => onLabelsChange(item.id, newLabels)
                            : undefined
                        }
                        onRename={() =>
                          onRenameClick(item.id, getSessionTitle(item))
                        }
...
interface SessionListProps {
  ...
  sessionOptions?: Map<
    string,
    import('../../hooks/useSessionOptions').SessionOptions
  >
...
export function SessionList({
  ...
  const currentFilter = isChatsNavigation(navState)
    ? navState.filter
    : undefined
  ...
  const loadMore = useCallback(() => {
    setDisplayLimit((prev) =>
      Math.min(prev + BATCH_SIZE, searchFilteredItems.length),
    )
  ...
  const dateGroups = useMemo(
    () => groupSessionsByDate(paginatedItems),
    [paginatedItems],
  )
  ...
  const handleFlagWithToast = useCallback(
    (sessionId: string) => {
      if (!onFlag) return
      onFlag(sessionId)
  ...
  const handleUnflagWithToast = useCallback(
    (sessionId: string) => {
      if (!onUnflag) return
      onUnflag(sessionId)
      toast('Flag removed', {
  ...
  const { activeIndex, setActiveIndex, getItemProps, focusActiveItem } =
    useRovingTabIndex({
      items: flatItems,
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent, _item: SessionMeta) => {
      if (e.key === 'ArrowLeft') {
        e.preventDefault()
        focusZone('sidebar')
        return
                  ...
                  <SessionItem
                    ...
                    onSelect={() => {
                      ...
                      if (!currentFilter || currentFilter.kind === 'allChats') {
                        ...
                        navigate(
                          routes.view.state(currentFilter.stateId, item.id),
                        )
                      }
                      // Notify parent
                    ...
                    permissionMode={
                      sessionOptions?.get(item.id)?.permissionMode
                    }
