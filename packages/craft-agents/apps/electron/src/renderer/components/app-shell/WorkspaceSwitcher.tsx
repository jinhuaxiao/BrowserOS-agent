...
import { Check, ChevronDown, ExternalLink, FolderPlus } from 'lucide-react'
...
export function WorkspaceSwitcher({
  ...
  const [iconCache, setIconCache] = useState<
    Record<string, { dataUrl: string; sourceUrl: string }>
  >({})
  ...
  useEffect(() => {
    const fetchIcons = async () => {
      for (const workspace of workspaces) {
        ...
        if (
          workspace.iconUrl?.startsWith('http://') ||
          workspace.iconUrl?.startsWith('https://')
        )
          continue
        ...
        try {
          ...
          if (result) {
            ...
            setIconCache((prev) => ({
              ...prev,
              [workspace.id]: { dataUrl, sourceUrl: workspace.iconUrl! },
            }))
          }
        } catch (error) {
          console.error(
            `Failed to load icon for workspace ${workspace.id}:`,
            error,
          )
        }
      }
      ...
      <DropdownMenu>
        ...
        <DropdownMenuTrigger asChild>
          ...
          >
            ...
            <CrossfadeAvatar
              src={
                selectedWorkspace ? getIconUrl(selectedWorkspace) : undefined
              }
            ...
            {!isCollapsed && (
              <>
                <FadingText
                  className="ml-1 font-sans min-w-0 text-sm"
                  fadeWidth={36}
                >
            ...
            >
              <div className="flex items-center gap-3 font-sans">
                <CrossfadeAvatar
                  ...
                  fallbackClassName="bg-foreground/5 text-xs rounded-full"
