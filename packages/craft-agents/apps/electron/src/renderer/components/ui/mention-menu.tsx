...
const MENU_ITEM_STYLE =
  'flex cursor-pointer select-none items-center gap-3 rounded-[6px] mx-1 px-2 py-1.5 text-[13px]'
const MENU_ITEM_SELECTED = 'bg-foreground/5'
// Type badge shown to the right of each item label (e.g. "Skill", "Source")
const MENU_TYPE_BADGE =
  'rounded-[4px] shadow-[0_0_0_1px_var(--shadow-tinted)] shadow-minimal bg-background px-1.5 py-0.5 text-[10px] text-foreground/50 shrink-0'
 ...
 */
export function isValidMentionTrigger(
...
export function InlineMentionMenu({
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-1.5 text-[12px] font-medium text-foreground/50 border-b border-border">
          ...
          return (
            ...
            >
              ...
              <div className="shrink-0">
                ...
                {item.type === 'file' && <FileMenuIcon name={item.label} />}
              </div>
                ...
                <>
                  ...
                  {item.file?.relativePath &&
                    getParentDir(item.file.relativePath) && (
                      <FadingText
                        className="text-[11px] text-foreground/50 min-w-0 opacity-50"
                        fadeWidth={20}
                      >
                        {getParentDir(item.file.relativePath)}
                      </FadingText>
...
export function useInlineMention({
  ...
  const fileSearchTimeout = React.useRef<ReturnType<typeof setTimeout> | null>(
    null,
  )
  ...
  const handleInputChange = React.useCallback(
    (value: string, cursorPosition: number) => {
      ...
      const isValidTrigger =
        atMatch && isValidMentionTrigger(textBeforeCursor, matchStart)
        ...
        if (basePath && filterText.length >= 1) {
          if (fileCache.current.length > 0) {
            ...
            fileSearchTimeout.current = setTimeout(async () => {
              try {
                window.electronAPI.debugLog(
                  '[mention] calling IPC searchFiles:',
                  basePath,
                  filterText,
                )
                ...
                window.electronAPI.debugLog(
                  '[mention] IPC returned:',
                  results?.length,
                  'results',
                )
                fileCache.current = results
                const filtered = filterCacheResults(
                  fileCache.current,
                  filterText,
                )
                window.electronAPI.debugLog(
                  '[mention] after cache filter:',
                  filtered.length,
                  'items',
                )
                setFileResults(filtered)
          ...
          window.electronAPI.debugLog(
            '[mention] skipping file search (no basePath or empty filter)',
          )
          if (fileSearchTimeout.current) {
            clearTimeout(fileSearchTimeout.current)
            fileSearchTimeout.current = null
    ...
    (item: MentionItem): { value: string; cursorPosition: number } => {
      ...
      if (atStart >= 0) {
        ...
        if (item.type === 'skill') {
          ...
          const qualifiedName = workspaceId
            ? `${workspaceId}:${item.id}`
            : item.id
