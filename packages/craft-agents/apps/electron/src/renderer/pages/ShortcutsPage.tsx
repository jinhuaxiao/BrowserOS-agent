...
function Kbd({
  ...
  return (
    <kbd
      className={`inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[11px] font-medium bg-foreground/5 border border-border rounded shadow-sm ${className || ''}`}
    >
      {children}
    </kbd>
  )
...
export default function ShortcutsPage() {
  return (
    <div className="h-full flex flex-col">
      <PanelHeader
        title="Shortcuts"
        actions={<HeaderMenu route={routes.view.settings('shortcuts')} />}
      />
      <Separator />
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        {shortcut.keys.map((key, keyIndex) => (
                          <Kbd
                            key={keyIndex}
                            className="group-hover:bg-foreground/10 group-hover:border-border"
                          >
                            {key}
                          </Kbd>
                        ))}
