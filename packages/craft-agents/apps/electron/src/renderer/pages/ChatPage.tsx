...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  const currentTodoState =
    session?.todoState || sessionMeta?.todoState || 'todo'
  const hasMessages = !!(
    session?.messages?.length || sessionMeta?.lastFinalMessageId
  )
              ...
              <StyledDropdownMenuItem
                onClick={() =>
                  window.electronAPI.openUrl(
                    'https://agents.craft.do/docs/go-further/sharing',
                  )
                }
              >
          ...
          <div className="h-full flex flex-col">
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <ChatDisplay
                ...
                onUltrathinkChange={(enabled) =>
                  setOption('ultrathinkEnabled', enabled)
                }
