...
export function parseCompoundRoute(route: string): ParsedCompoundRoute | null {
  ...
  // Browser Profiles navigator
  if (first === 'browser-profiles') {
    if (segments.length === 1) {
      return { navigator: 'browser-profiles' as NavigatorType, details: null }
    }

    // browser-profiles/profile/{profileId}
    if (segments[1] === 'profile' && segments[2]) {
      return {
        navigator: 'browser-profiles' as NavigatorType,
        details: { type: 'profile', id: segments[2] },
      }
    }
...
function convertCompoundToViewRoute(
  ...
  // Browser Profiles
  if (compound.navigator === 'browser-profiles') {
    if (!compound.details) {
...
function convertCompoundToNavigationState(
  ...
  if (compound.navigator === 'browser-profiles') {
    ...
    return {
      ...
      details: { type: 'profile', profileId: compound.details.id },
    }
  }
