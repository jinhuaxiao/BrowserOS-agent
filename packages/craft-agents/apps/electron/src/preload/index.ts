...
const api: ElectronAPI = {
  ...
  // Browser Profiles
  listBrowserProfiles: () => ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_LIST),
  getBrowserProfile: (profileId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_GET, profileId),
  createBrowserProfile: (input: import('../shared/types').CreateProfileInput) =>
    ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_CREATE, input),
  updateBrowserProfile: (profileId: string, input: import('../shared/types').UpdateProfileInput) =>
    ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_UPDATE, profileId, input),
  deleteBrowserProfile: (profileId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_DELETE, profileId),
  launchBrowserProfile: (profileId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_LAUNCH, profileId),
  stopBrowserProfile: (profileId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_STOP, profileId),
  ...
  ) => ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_REGENERATE_FINGERPRINT, profileId, options),
  getRunningBrowserProfiles: () =>
    ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_GET_RUNNING),
  batchCreateBrowserProfiles: (inputs: import('../shared/types').CreateProfileInput[]) =>
  ...
  getProxy: (proxyId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_GET, proxyId),
  createProxy: (input: import('../shared/types').CreateProxyInput) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_CREATE, input),
  updateProxy: (proxyId: string, input: import('../shared/types').UpdateProxyInput) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_UPDATE, proxyId, input),
  deleteProxy: (proxyId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_DELETE, proxyId),
  importProxies: (
    lines: string[],
    options?: { defaultType?: 'socks5' | 'http' | 'https'; tags?: string[]; region?: string; provider?: string }
  ) => ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_IMPORT, lines, options),
  checkProxyHealth: (proxyId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_CHECK_HEALTH, proxyId),
  checkAllProxiesHealth: () =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_CHECK_ALL_HEALTH),
  getProfilesUsingProxy: (proxyId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_GET_PROFILES_USING, proxyId),
  testProxyConnection: (config: { host: string; port: number }) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_TEST_CONNECTION, config),
  detectProxyGeoLocation: (proxyId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROXY_POOL_DETECT_GEO, proxyId),
  ...
  getProfileGroup: (groupId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_GROUPS_GET, groupId),
  createProfileGroup: (input: import('../shared/types').CreateGroupInput) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_GROUPS_CREATE, input),
  updateProfileGroup: (groupId: string, input: import('../shared/types').UpdateGroupInput) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_GROUPS_UPDATE, groupId, input),
  deleteProfileGroup: (groupId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_GROUPS_DELETE, groupId),
  getProfilesInGroup: (groupId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_GROUPS_GET_PROFILES, groupId),
  moveProfileToGroup: (profileId: string, groupId: string | undefined) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_GROUPS_MOVE_PROFILE, profileId, groupId),

  // Profile Templates
  listProfileTemplates: () => ipcRenderer.invoke(IPC_CHANNELS.PROFILE_TEMPLATES_LIST),
  getProfileTemplate: (templateId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_TEMPLATES_GET, templateId),
  createProfileTemplate: (input: import('../shared/types').CreateTemplateInput) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_TEMPLATES_CREATE, input),
  updateProfileTemplate: (templateId: string, input: import('../shared/types').UpdateTemplateInput) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_TEMPLATES_UPDATE, templateId, input),
  deleteProfileTemplate: (templateId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_TEMPLATES_DELETE, templateId),
  createProfileFromTemplate: (templateId: string, overrides?: Partial<import('../shared/types').CreateProfileInput>) =>
    ipcRenderer.invoke(IPC_CHANNELS.PROFILE_TEMPLATES_CREATE_PROFILE, templateId, overrides),
  batchCreateFromTemplate: (
    templateId: string,
    count: number,
    options?: { namePrefix?: string; groupId?: string; proxyIds?: string[] }
  ) => ipcRenderer.invoke(IPC_CHANNELS.PROFILE_TEMPLATES_BATCH_CREATE, templateId, count, options),

  // Migration
  migrateToProxyPool: () => ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_MIGRATE),
  needsProxyPoolMigration: () => ipcRenderer.invoke(IPC_CHANNELS.BROWSER_PROFILES_NEEDS_MIGRATION),

  // Browser Settings
  getBrowserSettings: () => ipcRenderer.invoke(IPC_CHANNELS.BROWSER_SETTINGS_GET),
  setBrowserSettings: (path: string, options?: { browserType?: string; useCustomPathOnly?: boolean }) =>
    ipcRenderer.invoke(IPC_CHANNELS.BROWSER_SETTINGS_SET, path, options),
  clearBrowserSettings: () => ipcRenderer.invoke(IPC_CHANNELS.BROWSER_SETTINGS_CLEAR),
  listAvailableBrowsers: () => ipcRenderer.invoke(IPC_CHANNELS.BROWSER_SETTINGS_LIST_AVAILABLE),
