...
function EditRequestBadge({ badge }: { badge: ContentBadge }) {
  ...
  return (
    <span className="inline-flex items-center h-[28px] px-2.5 rounded-[8px] bg-background shadow-minimal text-[13px] text-foreground/50">
      {displayLabel}
    </span>
  )
}
...
function ContextBadge({ badge }: { badge: ContentBadge }) {
  ...
  return (
    ...
    >
      ...
      <span className="truncate max-w-[200px] text-foreground/50">
        {displayLabel}
      </span>
    </span>
  )
...
export function UserMessageBubble({
  ...
  return (
    <div className={cn('flex flex-col items-end gap-3 w-full', className)}>
      ...
      {/* Queued badge */}
      {isQueued && (
        <span className="text-[10px] text-foreground/50 bg-foreground/5 px-2 py-0.5 rounded-full">
          queued
        </span>
      )}
    </div>
