...
import { FONTS, DEFAULT_FONTS } from './fingerprint-data.ts';
...
function normalizeFontFileNameToFamily(fileName: string): string | null {
  const raw = basename(fileName, fileName.includes('.') ? fileName.slice(fileName.lastIndexOf('.')) : '');
  const noStyle = raw
    .replace(/[-_]/g, ' ')
    .replace(/\b(?:thin|extralight|ultralight|light|regular|book|roman|medium|semibold|demibold|bold|extrabold|ultrabold|black|heavy|italic|oblique|condensed|narrow)\b/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
function getGlobalFontPool(): string[] {
  if (!cachedInstalledFontPool) {
    ...
    cachedInstalledFontPool = Array.from(new Set([...GLOBAL_FONT_POOL, ...installed]));
  }
...
function seededFontSample(
  ...
  let state = parseInt(createHash('sha256').update(seedKey).digest('hex').slice(0, 8), 16) >>> 0;
...
function normalizeKernelFonts(fingerprint: FingerprintConfig): {
  ...
  const configured = fingerprint.fonts ?? { blockFontEnumeration: false, enabledFonts: [] };
  ...
  const targetCount = Math.min(
    globalPool.length,
    Math.max(MIN_TARGET_FONT_COUNT, Math.min(MAX_TARGET_FONT_COUNT, keepSet.size + 280))
  );
  ...
  if (normalizedSet.size < targetCount) {
    ...
    const fill = seededFontSample(`${fingerprint.profileId}:font-fill`, fillPool, fillCount);
  ...
  const removablePool = Array.from(normalizedSet).filter((font) => !keepSet.has(font));
  const dropCount = Math.min(
    removablePool.length,
    Math.max(2, Math.min(8, Math.floor(removablePool.length * 0.03)))
  );
  const toDrop = new Set(
    seededFontSample(`${fingerprint.profileId}:font-drop`, removablePool, dropCount)
  );
...
export function fingerprintToKernelConfig(
  ...
  const badgeColor = options?.badge?.color ??
    (options?.platform ? getPlatformColor(options.platform) : getPlatformColor());
...
export function fingerprintToChromiumJson(
  ...
  return {
    ...
    mediaDevices: mediaDevices.length > 0 ? { devices: mediaDevices } : undefined,
 ...
 */
function hashFingerprintConfig(fingerprint: FingerprintConfig, options?: WriteBrowserOSConfigOptions): string {
...
export function writeBrowserOSConfigCached(
  ...
  const cacheResult = configCache.get<string>(cacheKey, { fingerprint, options });
...
export function isBrowserOSAvailable(): boolean {
  ...
  const browserOSPaths: Record<string, string[]> = {
    ...
    win32: ['C:\\Program Files\\BrowserOS\\BrowserOS.exe'],
  };
