...
export const NOVA_SELLER_BRAND = {
  ...
  /** Product description */
  description:
    'Professional fingerprint browser for Amazon sellers and e-commerce multi-account management',
...
export function getNovaSellerArgs(options: NovaSellerLaunchOptions): string[] {
  ...
  if (options.fingerprintConfigPath) {
    ...
    try {
      const base64 = readFileSync(options.fingerprintConfigPath).toString(
        'base64',
      )
      if (base64) {
