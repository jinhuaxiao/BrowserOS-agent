...
import type { DynamicValue, ExecutionResult } from './types.ts'
import { getWorkflow, recordWorkflowExecution } from './workflow-storage.ts'
import type {
  BrowserWorkflow,
  NodeExecutionResult,
  ParameterValue,
 ...
 */
export function addWorkflowEngineEventListener(
 ...
 */
export function removeWorkflowEngineEventListener(
  listener: WorkflowEventListener,
): void {
...
export class WorkflowEngine {
  ...
  private async executeNode(
    ...
    try {
      ...
      switch (node.type) {
        ...
        case 'workflow_call':
          nodeResult = await this.executeWorkflowCallNode(
            node,
            context,
            mcpClient,
          )
      ...
      if (node.retries && node.retries > 0) {
        for (let i = 0; i < node.retries; i++) {
          ...
          try {
            ...
            nodeResult = await this.executeSkillCallNode(
              node,
              context,
              mcpClient,
            )
    ...
    if (!nodeResult.success && !node.optional) {
      if (node.onError?.strategy === 'skip') {
        ...
        node.onError?.strategy === 'fallback' &&
        node.onError.fallbackStepId
      ) {
        // Jump to fallback node
        const fallbackNode = workflow.nodes.find(
          (n) => n.id === node.onError?.fallbackStepId,
        )
        if (fallbackNode) {
          await this.executeNode(
            fallbackNode,
    ...
    if (node.type !== 'end' && node.type !== 'condition') {
      ...
      for (const edge of nextEdges) {
        ...
        if (
          edge.condition &&
          !this.evaluateExpression(edge.condition, context)
        ) {
          continue
        }
  ...
  private async executeParallelNode(
    ...
    const maxConcurrency =
      config.maxConcurrency || this.config.maxConcurrentNodes
  ...
  private async executeClusterNode(
        ...
        const promise = (async () => {
          try {
            ...
            if (config.profileOverrides?.[profileId]) {
              parameters = {
                ...parameters,
                ...config.profileOverrides[profileId],
              }
            }
    ...
    // Aggregate results
    const successCount = Object.values(profileResults).filter(
      (r) => r.success,
    ).length
    const aggregatedResult = this.aggregateClusterResults(
      profileResults,
      config.aggregation,
    )
    ...
    return {
      ...
      durationMs: Math.max(
        ...Object.values(profileResults).map((r) => r.durationMs),
      ),
    }
  }
  ...
  private async executeTransformNode(
    ...
    try {
      ...
      const inputs = config.inputs.map((input) =>
        this.resolveValue(input, context),
      )
      ...
      switch (config.transformType) {
        ...
        case 'reduce':
          result = (inputs[0] as unknown[]).reduce((acc, item) =>
            this.evaluateExpressionWithItem(config.expression, context, {
              acc,
              item,
            }),
          )
  ...
  private mapParameters(
    mapping: Record<string, ParameterValue>,
    context: WorkflowExecutionContext,
