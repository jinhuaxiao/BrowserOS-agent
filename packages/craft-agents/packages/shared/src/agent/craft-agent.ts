...
import { type ConfigWatcher, createConfigWatcher } from '../config/watcher.ts'
import {
  getDateTimeContext,
  getSystemPrompt,
...
import { type AgentError, parseError } from './errors.ts'
import {
  blockWithReason,
  cleanupModeState,
  formatSessionState,
 ...
 */
function buildWindowsSkillsDirError(
  errorText: string,
): { type: 'typed_error'; error: AgentError } | null {
  ...
  const missingPath =
    pathMatch?.[1] || 'C:\\ProgramData\\ClaudeCode\\.claude\\skills'
...
export class CraftAgent {
  ...
  async *chat(
    ...
    try {
      ...
      if (activeBaseUrl) {
        debug(
          `[chat] Custom provider: baseUrl=${activeBaseUrl}, model=${model}, hasApiKey=${!!process.env.ANTHROPIC_API_KEY}`,
        )
      }
                ...
                async (input) => {
                      ...
                      if (!builtInMcpServers.has(serverName)) {
                        ...
                        if (!isActive) {
                          ...
                          if (sourceExists && this.onSourceActivationRequest) {
                            ...
                            try {
                              const activated =
                                await this.onSourceActivationRequest(serverName)
                              if (activated) {
                                this.onDebug?.(
                  ...
                  if (filePathTools.has(input.tool_name)) {
                    ...
                    if (configWriteTools.has(input.tool_name)) {
                      ...
                      const resolvedPath = (updatedInput?.file_path ??
                        toolInput.file_path) as string | undefined

                      if (resolvedPath) {
                        const detection = detectConfigFileType(
                          resolvedPath,
                          this.workspaceRootPath,
                        )
                        ...
                        if (detection) {
                          ...
                          if (input.tool_name === 'Write') {
                            ...
                            try {
                              ...
                              const replaceAll = toolInput.replace_all as
                                | boolean
                                | undefined
                          ...
                          if (contentToValidate) {
                            const validationResult = validateConfigFileContent(
                              detection,
                              contentToValidate,
                            )
                            if (validationResult && !validationResult.valid) {
                              this.onDebug?.(
                                `Config validation blocked ${input.tool_name} to ${detection.displayFile}: ${validationResult.errors.length} errors`,
                              )
                              return {
                                continue: false,
                                decision: 'block' as const,
                  ...
                  if (input.tool_name === 'Skill') {
                    ...
                    if (toolInput.skill && !toolInput.skill.includes(':')) {
                      ...
                      this.onDebug?.(
                        `Skill tool: qualified "${toolInput.skill}" → "${qualifiedSkill}"`,
                      )
                      return {
                        continue: true,
                        hookSpecificOutput: {
                  ...
                  if (!builtInTools.has(input.tool_name)) {
                    ...
                    >
                    const hasMetadata =
                      '_intent' in toolInput || '_displayName' in toolInput
      ...
      try {
        ...
        const stderrForConfigCheck = this.lastStderrOutput
          .join('\n')
          .toLowerCase()
        ...
        if (isConfigCorruption && !_isRetry) {
          debug(
            '[CraftAgent] Detected .claude.json corruption, repairing and retrying...',
          )
        ...
        if (isProcessError) {
          ...
          const windowsSkillsError = buildWindowsSkillsDirError(
            stderrContext || rawErrorMsg,
          )
  ...
  private formatSourceState(): string {
    ...
    for (const s of this.allSources) {
      this.knownSourceSlugs.add(s.config.slug)
    }
  ...
  private mapSDKErrorToTypedError(errorCode: SDKAssistantMessageError): {
    ...
    const errorMap: Record<SDKAssistantMessageError, AgentError> = {
      authentication_failed: {
        ...
        message:
          'Unable to authenticate with Anthropic. Your API key may be invalid or expired.',
        details: [
      ...
      server_error: {
        ...
        message:
          'Unable to connect to the API server. Check your internet connection.',
        details: [
      ...
      max_output_tokens: {
        ...
        message:
          'The model reached the maximum output token limit for this request.',
        details: [
        ...
        if (message.tool_use_result !== undefined) {
          ...
          if (toolUseId) {
            ...
            if (toolUse?.name === 'Task' && toolUseId) {
              ...
              parentToChildren.delete(toolUseId)
              console.log(
                `[CraftAgent] PARENT CLEANED UP (task-result): ${toolUseId}`,
              )
            }
