...
export interface SurfaceColors {
  ...
  /**
   * Border radius for UI components (e.g., "0.5rem", "4px")
   * Overrides --radius CSS variable
   */
  radius?: string
}
  ...
  if (override.dark) {
    ...
    for (const key of ALL_COLOR_KEYS) {
      if (override.dark[key] !== undefined) {
        // biome-ignore lint/style/noNonNullAssertion: dark is assigned above
...
export const BACKGROUND_HEX = {
  light: '#f5f5f0', // matches oklch(0.965 0.007 90)
  dark: '#111110', // matches oklch(0.145 0.005 90)
} as const
...
export const DEFAULT_THEME: ThemeOverrides = {
  background: 'oklch(0.965 0.007 90)',
  ...
  dark: {
    background: 'oklch(0.145 0.005 90)',
