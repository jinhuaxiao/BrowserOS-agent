...
import {
  BUNDLED_CONFIG_DEFAULTS,
  type ConfigDefaults,
} from './config-defaults-schema.ts'
...
import type { AuthType, Workspace } from '@craft-agent/core/types'
...
export function loadStoredConfig(): StoredConfig | null {
  try {
    ...
    const activeWorkspace = config.workspaces.find(
      (w) => w.id === config.activeWorkspaceId,
    )
    if (!activeWorkspace) {
...
export function getActiveWorkspace(): Workspace | null {
  ...
  return (
    config.workspaces.find((w) => w.id === config.activeWorkspaceId) ||
    config.workspaces[0] ||
    null
  )
}
...
export function saveWorkspaceConversation(
  ...
  try {
    ...
    console.error(
      `[storage] [CYCLIC STRUCTURE] Failed to save workspace conversation:`,
      e,
    )
    console.error(
      `[storage] Message count: ${messages.length}, message types: ${messages.map((m) => m.type).join(', ')}`,
    )
    ...
    try {
      const sanitizedMessages = messages.map((m, i) => {
        ...
        if (m.toolInput) {
          try {
            ...
            console.error(
              `[storage] [CYCLIC STRUCTURE] in message ${i} toolInput (tool: ${m.toolName}), keys: ${Object.keys(m.toolInput).join(', ')}, error: ${inputErr}`,
            )
      ...
      console.error(
        `[storage] Saved sanitized workspace conversation successfully`,
      )
    } catch (e2) {
      console.error(
        `[storage] Failed to save even sanitized workspace conversation:`,
        e2,
      )
    }
  }
...
export function loadWorkspaceConversation(
  workspaceId: string,
...
export function ensurePresetThemes(bundledThemesDir?: string): void {
  ...
  try {
    const bundledFiles = readdirSync(bundledThemesDir).filter((f) =>
      f.endsWith('.json'),
    )
 ...
 */
function resolveThemeBackgroundImage(
  theme: ThemeFile,
  themePath: string,
): ThemeFile {
  ...
  try {
    ...
    console.warn(
      `Failed to read theme background image: ${absoluteImagePath}`,
      error,
    )
    return theme
  }
 ...
 * @param bundledThemesDir - Path to bundled themes (e.g., Electron's resources/themes)
 */
export function resetPresetTheme(
  id: string,
