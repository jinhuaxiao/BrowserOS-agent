...
export function ProxyCard({
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    if (!showDeleteConfirm) {
      ...
      const profileIds = await window.electronAPI.getProfilesUsingProxy(proxy.id);
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {/* Type badge */}
            <span className="px-1.5 py-0.5 text-xs rounded bg-muted uppercase">
              {proxy.type}
            </span>
          </div>
          ...
          {proxy.geoLocation && (
            <div className="flex items-center gap-2 mt-2 text-sm">
              <span className="text-lg">{getCountryFlag(proxy.geoLocation.country)}</span>
              <span>
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              ...
              {proxy.profileCount} {proxy.profileCount === 1 ? 'profile' : 'profiles'}
            </span>

            {/* Region (only show if no geoLocation) */}
            {!proxy.geoLocation && proxy.region && (
              <span className="uppercase">{proxy.region}</span>
            )}

            {/* Response time */}
            {proxy.responseTimeMs !== undefined && (
              <span>{proxy.responseTimeMs}ms</span>
            )}
        ...
        <div className="flex items-center gap-1 ml-2">
          <Button
            ...
            title={proxy.geoLocation ? 'Refresh geolocation' : 'Detect geolocation'}
          >
            {isDetectingGeo ? (
          ...
          <Button
            ...
            title={showDeleteConfirm ? 'Click again to confirm delete' : 'Delete proxy'}
          >
            {isDeleting ? (
              <Loader2Icon className="w-4 h-4 animate-spin" />
            ) : (
      ...
      {showDeleteConfirm && (
        <div className="mt-3 p-2 bg-red-500/10 rounded text-sm">
          <p className="text-red-500">
            This proxy is used by {proxy.profileCount} profile(s). Click delete again to
            confirm.
          </p>
          <Button
            variant="ghost"
